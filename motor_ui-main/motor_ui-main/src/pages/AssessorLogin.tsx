...
export default function AssessorLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    // Simple login - store assessor info and navigate
    localStorage.setItem("assessorId", assessorId.toUpperCase());
    localStorage.setItem("assessorName", "Assessor");
    setTimeout(() => {
      setLoading(false);
      ...
      <main className="flex-1 flex items-center justify-center p-4 md:p-8">
        <div className="w-full max-w-[1000px] grid grid-cols-1 md:grid-cols-2 bg-card rounded-xl overflow-hidden shadow-xl shadow-primary/5 border border-primary/10">
          <div className="relative hidden md:block overflow-hidden">
            ...
            <div className="h-full w-full bg-cover bg-center" style={{ backgroundImage: "url('https://lh3.googleusercontent.com/aida-public/AB6AXuBcpDiqhxnZnMZXibhtIqDOfGGzchVVnXEk8tcgFk9lhYMGzSQVqozajgd9UDpWQhMAv0oNX_EYgbxEvhNOHWESea-Z_BDJteNX930v6SpvlogS5DtoHhTHEnJhBTBU4GbLEPRsAvZi8uG3nxHQ1gx84iLNo0UahNa1D6LbHYSK9H5B7Uy9_lLgautu0mikZAYkHoVGiwk1JqqHIwy187q9I1oD7ABLUSgtu1i5ZEjMavUNlIk8XC-QzAZi2CZ-epSmn2DIjNaAa6w')" }} />
            <div className="absolute bottom-0 left-0 p-10 z-20">
              <h1 className="text-primary-foreground text-3xl font-bold mb-2">Claims Intelligence AI</h1>
              <p className="text-white/90 text-sm max-w-xs mb-6">Harnessing AI-driven insights for precise risk assessment.</p>
              <div className="flex items-center gap-2 text-white/90 text-sm font-medium">
                <span className="material-symbols-outlined text-[18px]">verified</span>
                Official Assessor Portal
              </div>
            </div>
          </div>
          ...
          <div className="p-8 md:p-12 flex flex-col justify-center">
            <div className="mb-8">
              <div className="flex items-center gap-2 mb-4">
                <span className="bg-primary/10 text-primary text-[10px] font-bold px-2 py-1 rounded uppercase tracking-widest">Assessor Access</span>
              </div>
              <h2 className="text-foreground text-3xl font-bold mb-2">Assessor Portal</h2>
              <p className="text-muted-foreground text-sm">Secure access for authorized vehicle damage assessors.</p>
            </div>
            <form className="space-y-5" onSubmit={handleSubmit}>
              <div className="flex flex-col gap-2">
                <label className="text-foreground text-sm font-semibold">Assessor ID</label>
